...
export class UniAppFrameworkAdapter implements FrameworkAdapter {
  ...
  async build(options: BuildOptions): Promise<void> {
    ...
    try {
      ...
      if (errorMessage.includes('uni-app') || errorMessage.includes('cli')) {
        options.logger.error('[uni-app] 提示: 请确保已安装 uni-app CLI 或配置了构建脚本');
      } else if (errorMessage.includes('vue.config')) {
        options.logger.error('[uni-app] 提示: 请检查 vue.config.js 配置');
      } else if (errorMessage.includes('dependencies')) {
        options.logger.error('[uni-app] 提示: 请检查项目依赖是否已正确安装');
      } else if (errorMessage.includes('HBuilderX')) {
        options.logger.error('[uni-app] 提示: 如使用 HBuilderX，请确保已正确配置 CLI 工具');
      }
    ...
    try {
      ...
      try {
        ...
        if (vueConfig?.default?.pluginOptions?.['uni-app']?.outputDir) {
          outputDir = vueConfig.default.pluginOptions['uni-app'].outputDir;
